"""
ICT Strategy Backtest — Time Window Comparison
===============================================
Tests multiple trade entry windows and shows which produces the best P&L.
...
Windows tested:
  ...
  6:30 AM – 11:00 AM PT
  6:30 AM – 12:00 PM PT
  7:00 AM –  9:00 AM PT  ← current live setting
  7:00 AM – 10:00 AM PT
  7:00 AM – 11:00 AM PT
  8:00 AM – 10:00 AM PT
  8:00 AM – 11:00 AM PT
...
# ── Fixed Settings (same as live bot) ──────────────────────
TICKER         = "QQQ"
CONTRACTS      = 2
PROFIT_TARGET  = 1.00
STOP_LOSS      = 0.60
...
OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "ICT_Window_Comparison.xlsx")
CSV_FILE    = os.path.join(os.path.dirname(__file__), "ICT_Window_Comparison.csv")

# ── Windows to test: (start_hour, start_min, end_hour, label) ──
WINDOWS = [
    (6, 30,  9, "6:30–9:00 AM"),
    (6, 30, 10, "6:30–10:00 AM"),
    (6, 30, 11, "6:30–11:00 AM"),
    (6, 30, 12, "6:30–12:00 PM"),
    (7,  0,  9, "7:00–9:00 AM  ← CURRENT"),
    (7,  0, 10, "7:00–10:00 AM"),
    (7,  0, 11, "7:00–11:00 AM"),
    (8,  0, 10, "8:00–10:00 AM"),
    (8,  0, 11, "8:00–11:00 AM"),
]
...
def simulate_exit(bars_5m, entry_bar, entry_opt_px, direction):
    ...
    for i in range(entry_bar + 1, min(entry_bar + 300, len(bars_5m))):
        ...
        if hit_tp or hit_sl or eod or time_exit:
            ...
            return {"result": result, "pnl_usd": pnl_usd, "pnl_pct": round(pnl_pct * 100, 1)}
...
def run_window(bars_5m, bars_1h, bars_4h, vix_by_date,
               trading_days, start_h, start_min, end_h):
    """Run the full backtest for one specific time window."""
    trades = []
    ...
    for day_date in trading_days:
        ...
        for sig in all_signals:
            ...
            in_window = (
                (et_pt.hour > start_h or (et_pt.hour == start_h and et_pt.minute >= start_min))
                and et_pt.hour < end_h
            )
            if not in_window:
                continue
            ...
            last_exit_bar = entry_bar + 18  # approx 90 min
...
def summarize(trades, label):
    if not trades:
        return {
            "Window": label, "Trades": 0, "Wins": 0, "Losses": 0, "Scratches": 0,
            "Win Rate": "N/A", "Total P&L": "$0.00", "Avg Win": "N/A", "Avg Loss": "N/A",
            "_total_pnl": 0, "_win_rate": 0, "_trades": 0,
        }
...
def main():
    print("=" * 65)
    print("  ICT Strategy — Time Window Comparison Backtest")
    print("  Settings: TP=100% | SL=60% | Trailing Stop ON | 90min exit")
    print("=" * 65)
    print()
    ...
    raw_5m      = fix_cols(raw_5m)
    raw_1h      = fix_cols(raw_1h)
    raw_1h_base = fix_cols(raw_1h_base)
    vix_raw     = fix_cols(vix_raw)
    ...
    for (start_h, start_min, end_h, label) in WINDOWS:
        print(f"  Testing {label}...", end="", flush=True)
        trades = run_window(bars_5m, bars_1h, bars_4h, vix_by_date,
                            trading_days, start_h, start_min, end_h)
    ...
    print(f"{'='*65}")
    print(f"  {'Window':<28} {'Trades':>6}  {'Win Rate':>9}  {'Total P&L':>12}")
    print(f"  {'-'*60}")
    for i, r in enumerate(results):
        marker = " ← BEST" if i == 0 else (" ← CURRENT" if "CURRENT" in r["Window"] else "")
        print(f"  {r['Window']:<28} {r['Trades']:>6}  {r['Win Rate']:>9}  {r['Total P&L']:>12}{marker}")

    print(f"\n  Best window: {results[0]['Window']}")
    print(f"  P&L: {results[0]['Total P&L']}  |  Win Rate: {results[0]['Win Rate']}  |  Trades: {results[0]['Trades']}")

    # ── Export ─────────────────────────────────────────────
    export_cols = ["Window", "Trades", "Wins", "Losses", "Scratches",
