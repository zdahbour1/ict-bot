...
from strategy.option_selector import select_and_enter
import config

log = logging.getLogger(__name__)


def create_app(client: TastytradeClient, exit_manager: ExitManager) -> Flask:
    app = Flask(__name__)
    ...
    def webhook():
        ...
        if signal == "bullish_sweep":
            trade = select_and_enter(client)
            if trade:
                exit_manager.add_trade(trade)
                return jsonify({"status": "trade_opened", "symbol": trade["symbol"]}), 200
            else:
