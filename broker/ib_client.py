...
def _normalize_occ(local_symbol: str | None) -> str:
    ...
    width (e.g. ``'QQQ   260420C00645000'``). Downstream code expects
    the canonical unpadded form ``'QQQ260420C00645000'``. A bare
    ``.strip()`` only trims leading/trailing whitespace — insufficient
    for tickers shorter than the width. Use this helper at every
    ``localSymbol`` read site.
    """
...
class IBClientCore:
    ...
    def __init__(self, connection=None, contract_cache=None, cache_lock=None,
                 pool=None):
        # Optional pool reference — used by cancel_order_by_id and
        # find_open_orders_for_contract to fan out across every client
        # in the pool. IB's cancelOrder only works on the client that
        # PLACED the order (error 10147 otherwise); ib_async's per-
        # connection openTrades cache also goes stale when a sibling
        # connection cancels an order. Pool fan-out fixes both.
        self._pool = pool
