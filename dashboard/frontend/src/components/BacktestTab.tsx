...
export default function BacktestTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Trades table — shown only when a run is selected */}
      {selectedRunId != null && (
        <div className="bg-[#161b22] border border-[#30363d] rounded-lg overflow-x-auto">
          <div className="px-4 py-3 border-b border-[#30363d] flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-gray-500 mt-0.5">
                ...
                {selectedRun && ` · P&L ${fmtUsd(selectedRun.total_pnl)} · ${Number(selectedRun.win_rate || 0).toFixed(1)}% win rate`}
              </div>
            </div>
            <div className="flex items-center gap-2">
              {(['all', 'WIN', 'LOSS'] as const).map(f => (
                <button key={f}
                        ...
                        className={`px-2 py-0.5 text-xs rounded ${
                          tradesFilter === f ? 'bg-blue-500/20 text-blue-400' : 'text-gray-500 hover:text-gray-300'
                        }`}>
                  {f === 'all' ? 'All' : f}
                </button>
              ))}
              ...
              <button onClick={() => setTradesPage(p => Math.max(0, p - 1))}
                      ...
                      className="px-2 py-0.5 text-xs rounded border border-[#30363d] text-gray-400 disabled:text-gray-700">Prev</button>
              <span className="text-xs text-gray-500">{tradesPage + 1} / {totalPages}</span>
              <button onClick={() => setTradesPage(p => Math.min(totalPages - 1, p + 1))}
                      disabled={tradesPage >= totalPages - 1}
                      className="px-2 py-0.5 text-xs rounded border border-[#30363d] text-gray-400 disabled:text-gray-700">Next</button>
            </div>
          </div>
            ...
            <tbody>
              ...
              {trades.length === 0 && (
                <tr><td colSpan={9} className="px-3 py-6 text-center text-gray-500">
                  {tradesTotal > 0 ? 'Loading trades or no matches on this page...' : 'No trades in this run.'}
                </td></tr>
              )}
            </tbody>
          </table>
